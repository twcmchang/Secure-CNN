...
def count_number_params(para_dict):
    """
    para_dict: the dictionary of network parameters
    
    return: a integer, number of parameters
    """
    n = 0
    for k,v in sorted(para_dict.items()):
        if 'bn_mean' in k:
...
def get_params_shape(shape):
    ...
    return: a integer, number of elements in the array
    """
    n = 1
    for dim in shape:
        n = n*dim
...
def count_flops(para_dict, net_shape):
    """
    para_dict: dictionary of network parameters (layer-wise)
    net_shape: input shape of each layer

    return: FLOPs, number of multiplication and addition operations
    """
...
def countFlopsParas(net):
    """
    net: a declared network model

    return: number of flops, number of parameters
    """
